...
char buffer[MAXLINE],ip4[INET6_ADDRSTRLEN];
    struct addrinfo servaddr, *servinfo,*p;
 int connfd,option=1,rv;
 ...
 if((rv=getaddrinfo(argv[1],SERV_PORT,&servaddr, &servinfo))!=0)
{
 ...
 for(p=servinfo;p!=NULL;p=p->ai_next)
{
 ...
 if(p==NULL)
    ...
    inet_ntop(p->ai_family,get_in_addr((struct sockaddr*)p->ai_addr),ip4,sizeof(ip4));
